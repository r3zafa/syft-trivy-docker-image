...
if [[ "$1" == "--help" || "$1" == "-h" ]]; then
  cat << EOF
Usage: ./publish.sh [--version VERSION]
...
Behavior:
  ...
  - Only the --version option is supported.
...
if [ -z "$DOCKER_VERSION" ]; then
  ...
  print_warning "Usage: ./publish.sh [--version VERSION]"
  print_warning "Try './publish.sh --help' for more information."
  exit 1
fi
...
# --- Check image exists, assume user is logged in ---
if ! docker image inspect "$DOCKER_IMAGE" >/dev/null 2>&1; then
  print_error "Image $DOCKER_IMAGE not found. Please build it first."
  exit 1
fi
