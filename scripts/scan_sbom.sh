...
mkdir -p /workspace/sbom/severity-scan
trivy sbom --ignore-unfixed --timeout 10m --severity "${failOnSeverity}" --format table "$SBOM" | tee /workspace/sbom/severity-scan/trivy-sbom.txt || true
trivy sbom --ignore-unfixed --timeout 10m --severity "${failOnSeverity}" --format sarif --output /workspace/sbom/severity-scan/trivy-sbom.sarif "$SBOM" || true
